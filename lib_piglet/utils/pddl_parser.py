        ...
        for t in re.findall(r"[()]|[^\s()]+", str):
            if t == "(":
                ...
                if stack:
                    ...
                    raise Exception(
                        "Missing open parentheses when parsing:" + list[0][-1].__str__()
                        if len(list) >= 1 and len(list[0]) >= 1
                        else "Missing open parentheses"
                    )
            else:
                list.append(t)
        if stack:
            raise Exception(
                "Missing close parentheses when parsing: " + stack[-1].__str__()
                if len(stack) >= 1
                else "Missing close parentheses"
            )
        if len(list) != 1:
