...
class pddl_state:
    ...
    def __repr__(self):
        if self.from_action_ is None:
            return str("START")
        return "{}: {}".format(
            self.from_action_.name, " ".join(self.from_action_.parameters)
        )
...
class pddl(base_domain[pddl_state]):
    ...
    def set_start_goal(self):
        ...
        self.goal_state_: pddl_goal = pddl_goal(
            self.parser_.positive_goals, self.parser_.negative_goals
        )
    ...
    def is_goal(self, current_state: pddl_state, goal_state: pddl_goal):
        return self.applicable(
            current_state.state_set_, goal_state.goal_pos_, goal_state.goal_neg_
        )
