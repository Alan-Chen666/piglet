...
from lib_piglet.constraints.grid_constraints import (
    grid_constraint_table,
    grid_reservation_table,
)
import copy
...
class grid_expander(base_expander):
    ...
    def __init__(self, map: gridmap, constraint_table: grid_constraint_table = None):
        ...
        self.reservation_table_: grid_reservation_table = (
            None  # reservation_table_ is not used on default, decide how to use it on your own.
        )
    ...
    def expand(self, current: search_node):
        ...
        for a in self.get_actions(current.state_):
            ...
            if self.constraint_table_ is not None:
                if self.constraint_table_.get_constraint(
                    new_state, current.timestep_ + 1
                ).v_:
                    continue
                if self.constraint_table_.get_constraint(
                    current.state_, current.timestep_
                ).e_[a.move_]:
                    continue
...
class grid_joint_expander(base_expander):
    ...
    def expand(self, current: search_node):
        ...
        self.generate_states(
            current_state,
            agents_left,
            len(current_state.agent_locations_),
            loc_set,
            current_state,
        )
    ...
    def generate_states(
        self,
        current_state: grid_joint_state,
        agents_left: list,
        cost: int,
        loc_set,
        parent_state,
        ...
        for a in self.get_actions(loc):
            ...
            if (
                loc in loc_set
                and parent_state.agent_locations_[loc_set[loc]] == new_loc
            ):
                continue
